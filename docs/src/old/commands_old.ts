import type { EventCommand, EventPage, MapEvent, PageCondition } from "../types";
...
function getFacePath(face: string): string {
    return "/faces/" + face.toLowerCase() + ".png"; // lower fixes at least one case of @Niko in OS16/map14
}
...
export function emitEvents(events: MapEvent[]): HTMLElement[] {
    const result: HTMLElement[] = [];
    for (const event of events) {
        const eventRoot = document.createElement("ul");
        ...
        result.push(eventRoot);
    }
    return result;
}

function emitPage(page: EventPage, index: number): HTMLElement {
    const pageRoot = document.createElement("li");
    pageRoot.innerText = "Page " + index;
    pageRoot.classList.add("page", "border");
...
function emitShowText(command: EventCommand): HTMLElement {
    ...
    if (portrait) result.appendChild(portrait);
